...
foreach ($days as &$d) {
    ...
    if ($d['time_in'] && $d['time_out']) {
        ...
        $total_completed += $d['hours_worked'];
    } else {
        $d['hours_worked'] = null;
    }
}
...
$user_stmt = $pdo->prepare('SELECT required_hours FROM users WHERE id = ?');
$user_stmt->execute([$_SESSION['user_id']]);
$required = $user_stmt->fetchColumn();
$required = $required !== null ? (float)$required : null;
$remaining = $required !== null ? round($required - $total_completed, 2) : null;
$is_done = $remaining !== null && $remaining <= 0;
$pct = ($required && $required > 0) ? min(100, round(($total_completed / $required) * 100)) : 0;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<style>
    * { box-sizing: border-box; }
    ...
    .main h2 { color: #0d2d4e; margin-top:0; margin-bottom: 20px; }

    .hours-summary {
        background: #fff; border-radius: 8px; padding: 18px 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.06);
        margin-bottom: 24px; display:flex; align-items:center; gap: 24px; flex-wrap: wrap;
    }
    .hours-summary .stat { text-align:center; }
    .hours-summary .stat .val { font-size: 22px; font-weight: bold; color: #0d2d4e; }
    .hours-summary .stat .val.done { color: #1e7a34; }
    .hours-summary .stat .lbl { font-size: 11px; color: #6b7280; text-transform:uppercase; }
    .hours-summary .progress-bar { flex:1; min-width: 160px; height: 10px; background: #eef0f3; border-radius: 5px; overflow: hidden; }
    .hours-summary .progress-bar .fill { height: 100%; background: #0ea5e9; }
    .hours-summary .progress-bar .fill.done { background: #1e7a34; }
...
<div class="layout">
    ...
    <div class="main">
        ...
        <div class="hours-summary">
            <div class="stat">
                <div class="val <?= $is_done ? 'done' : '' ?>"><?= number_format($total_completed, 1) ?></div>
                <div class="lbl">Completed</div>
            </div>
            <div class="progress-bar"><div class="fill <?= $is_done ? 'done' : '' ?>" style="width:<?= $pct ?>%;"></div></div>
            <div class="stat">
                <div class="val <?= $is_done ? 'done' : '' ?>"><?= $is_done ? 0 : number_format($remaining, 1) ?></div>
                <div class="lbl"><?= $is_done ? 'Completed!' : 'Remaining' ?></div>
            </div>
            <div class="stat">
                <div class="val" style="font-size:15px;"><?= number_format($required, 1) ?></div>
        ...
        <?php else: ?>
            <div class="empty-state">No DTR records yet. Tap in at the intern biometrics station to get started.</div>
        <?php endif; ?>
    </div>
</div>
</body>
