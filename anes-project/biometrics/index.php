<?php
// ============================================================
// Biometrics Kiosk — standalone page, meant to run on a tablet/PC
// placed near the entrance. This intentionally does NOT start a
// normal portal session — it only verifies credentials, captures
// a photo, and logs a Time In / Time Out entry.
// ============================================================
session_start();
...
$result = null; // holds the confirmation data to display
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $password === '') {
        ...
        if (!$user || $user['status'] !== 'active' || !password_verify($password, $user['password_hash'])) {
            ...
            // Determine whether this tap is a Time In or Time Out
            ...
            // Save the captured photo, if any
            $photo_filename = null;
...
<head>
<meta charset="UTF-8">
<title>Biometrics - Time In / Time Out</title>
<style>
    * { box-sizing: border-box; }
    html, body {
        margin: 0; height: 100%; font-family: 'Segoe UI', Arial, sans-serif;
        background: #1a3a6b; display:flex; align-items:center; justify-content:center;
    }
    .kiosk-card {
        width: 480px; max-width: 94%; background: #fff; border-radius: 10px;
        border-top: 5px solid #4a90d9; box-shadow: 0 20px 50px rgba(0,0,0,0.35);
    ...
    .camera-box video { transform: scaleX(-1); } /* mirror the live preview only — captured photo stays true/unmirrored */
    ...
    .submit-btn {
        ...
        background: #4a7fd4; color: #fff; font-size: 16px; font-weight: bold; cursor: pointer;
    }
    .submit-btn:hover { background: #3c6cc0; }
...
<?php else: ?>
    <div class="kiosk-card">
        ...
        <div class="subtitle">NKTI Anesthesiology &middot; Biometrics Station</div>
        ...
        <form method="POST" action="index.php" id="kioskForm">
