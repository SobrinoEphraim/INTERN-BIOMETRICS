...
if (isset($_GET['toggle_status'])) {
    ...
    $stmt = $pdo->prepare('SELECT status FROM exams WHERE id = ? AND created_by = ?');
    $stmt->execute([$id, $_SESSION['user_id']]);
    $current = $stmt->fetchColumn();
    if ($current) {
        $new = $current === 'published' ? 'draft' : 'published';
        $pdo->prepare('UPDATE exams SET status = ? WHERE id = ?')->execute([$new, $id]);
    }
...
if (isset($_GET['delete'])) {
    $id = (int) $_GET['delete'];
    $pdo->prepare('DELETE FROM exams WHERE id = ? AND created_by = ?')->execute([$id, $_SESSION['user_id']]);
    header('Location: exams.php');
    exit;
}
...
$exams = $pdo->prepare(
    'SELECT e.*,
        ...
        (SELECT COUNT(*) FROM exam_submissions s WHERE s.exam_id = e.id AND s.status = "submitted") AS submission_count
     FROM exams e
     WHERE e.created_by = ?
...
<style>
    * { box-sizing: border-box; }
    ...
    .exam-meta { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 14px; }
    .badge { padding: 4px 10px; border-radius: 12px; font-size: 12px; font-weight: bold; }
    .badge.published { background:#eaf7ee; color:#1e7a34; }
    .badge.draft { background:#f3f4f6; color:#6b7280; }
    .badge.role { background:#e5edfb; color:#2b5cad; }
...
<div class="layout">
    ...
    <div class="main">
        <div class="topbar">
            <h2>Exams</h2>
            <a class="btn" href="exam_builder.php">+ Create New Exam</a>
        </div>
        ...
        <div class="exam-grid">
            ...
            <div class="exam-card">
                ...
                <div class="exam-meta">
                    <span class="badge <?= $e['status'] ?>"><?= ucfirst($e['status']) ?></span>
                    <span class="badge role"><?= htmlspecialchars($role_labels[$e['target_role']] ?? $e['target_role']) ?></span>
                </div>

                <div class="exam-stats">
                    <?= (int)$e['question_count'] ?> question<?= $e['question_count'] == 1 ? '' : 's' ?>
                    &middot; <?= (int)$e['submission_count'] ?> submission<?= $e['submission_count'] == 1 ? '' : 's' ?>
                </div>

                <div class="exam-actions">
                    <a class="action-link" href="exam_builder.php?id=<?= (int)$e['id'] ?>">Edit</a>
                    <?php if ($e['status'] === 'published'): ?>
                        <a class="action-link unpublish" href="exams.php?toggle_status=<?= (int)$e['id'] ?>">Unpublish</a>
                    <?php else: ?>
                        <a class="action-link publish" href="exams.php?toggle_status=<?= (int)$e['id'] ?>">Publish</a>
                    <?php endif; ?>
                    <a class="action-link delete" href="exams.php?delete=<?= (int)$e['id'] ?>"
                       onclick="return confirm('Delete this exam and all its submissions? This cannot be undone.');">Delete</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
