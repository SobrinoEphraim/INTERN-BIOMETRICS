...
// Build a descriptive filename based on the active filters
$name_part = 'all-employees';
if ($employee_filter) {
    $emp_stmt = $pdo->prepare('SELECT full_name FROM users WHERE id = ?');
    $emp_stmt->execute([$employee_filter]);
...
$range_part = ($date_from || $date_to)
    ...
    : '';
$filename = 'DTR_' . $name_part . $range_part . '_' . date('Ymd_His') . '.csv';

header('Content-Type: text/csv; charset=utf-8');
header('Content-Disposition: attachment; filename="' . $filename . '"');

$out = fopen('php://output', 'w');
fputcsv($out, ['Employee', 'Date', 'Time In', 'Time Out']);
